    ...
    return {
        ...
        performAutoBinding: function (container, namingContext) {
            Dom.doOnChildRecursively(container, {
                ...
                parentNode.replaceChild(widget.node(), n);
                namingContext.__childWidgets.push(widget);

                if (parentNode.ownerDocument.body.contains(parentNode)) widget.signalOnAttached();
            });
        },
...
BaseWidget.prototype.signalOnAttached = function () {
    ...
    if (this.__childWidgets) {
        for (var i = 0; i < this.__childWidgets.length; i ++) {
            var o = this.__childWidgets[i];
            if (o && o.signalOnAttached) {
                o.signalOnAttached();
            }
        }
    }
...
BaseTemplatedWidget.prototype.upgradeMDL = function (node) {
    if (node.className && node.className.match(/^mdl\-.+/)) {
        console.log("upgrading: ", node.className, document.body.contains(node));
        ...
        if (node.childNodes) {
            for (var i = 0; i < node.childNodes.length; i ++) {
                ...
                if (!child.__widget) this.upgradeMDL(child);
            }
        }
    }
};
