widget.DataTable = function () {
        ...
        if (all) {
            if (checkbox.checked) {
                if (dataTable.totalItems > 200) {
                    widget.Dialog.error(Messages["selection_in_table_limit_msg"]);
                    checkbox.checked = false;
                    return;
                }
                if (dataTable.totalItems > 50) {
                    widget.Dialog.confirm(
                            String.format(Messages["you_are_going_to_select_n_items_msg"], dataTable.totalItems),
                            String.format(Messages["yes_select_n_items_msg"], dataTable.totalItems), function () {
                        dataTable.selectAll();
    ...
    function SelectorColumn(exclusive) {
    ...
    SelectorColumn.prototype.getCellContentHtml = function (data, row, col) {
        return "<input primary=\"true\" type=\"" + (this.exclusive ? "radio" : "checkbox") + "\" class=\"DataTableCheck Item\" data-index=\"" + row + "\" name=\"" + this.name + "\" />";
    };
    SelectorColumn.prototype.getTitleInfo = function () {
        return "Select all";
    };
    SelectorColumn.prototype.getTitleContentHtml = function () {
        return this.exclusive ? "" : "<input type=\"checkbox\" class=\"DataTableCheck All\" /><span class=\"DataTableCheckInfo\"><i class=\"fa fa fa-info\"></i></span>";
    };
    ...
    function DataTable(container, id) {
    ...
    DataTable.prototype.addSelectionInfoPane = function() {
        ...
        var selectionItemDataTable = new widget.DataTable(bodyPane);
        
        selectionItemDataTable.column(new widget.DataTable.PlainTextColumn(Messages["table_colum_header_index"], function(data, row, col){
            return (row +1);
        }).width("4em"));
        selectionItemDataTable.column(new widget.DataTable.PlainTextColumn(Messages["table_column_header_item"], function(data){
            return thiz.sumarizerSelectionItem(data);
        }).width("1*"));
        var removeAction = {
                id: "removeItem", type: "fa fa-trash", title: Messages["remove"],
        ...
        selectionItemDataTable.column(new widget.DataTable.ActionColumn(actions).width("6em"));
    ...
    DataTable.prototype.createColumnDefinitions = function () {
        ...
        if (this.withSelector) {
            var selectorColumn = new SelectorColumn(this.exclusive);
    ...
    DataTable.prototype._init = function() {
        ...
        for (var i = 0; i < this.actualColumns.length; i ++) {
            ...
            if (wText.endsWith("px")) {
    			...
    			var em = wText.substring(0, wText.length-2);
    			var m = document.getElementById("emMeasureDiv");
    			w = em * (m == null ? 10 : m.offsetWidth);
    			col._width = w;
            	remainingWidth -= w;
    ...
    DataTable.prototype.showColumnSettingDialog = function () {
        var thiz = this;
        var builder = {
            title: Messages["column_settings"],
            size: "mini",
	...
	DataTable.prototype.getDefaultRowHeight = function() {
		...
		if (this.items.length == 0) {
        	rowHeight = Dom.get("emMeasureDiv").offsetWidth * 1.5;
        } else {
