...
EpgzHandler.prototype.loadDocument = function(filePath) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        var wrappedReject = function(error) {
            ...
            console.log(error);
            var recoverable = fs.existsSync(
                path.join(Pencil.documentHandler.tempDir.name, "content.xml")
            );
            if (!recoverable) {
                reject(error);
            } else {
        ...
        fs
            ...
            .pipe(
                tarfs
                    .extract(Pencil.documentHandler.tempDir.name, {
                        readable: true,
                        writable: true
                    })
                    .on("error", wrappedReject)
...
EpgzHandler.prototype.saveDocument = function(documentPath) {
    return new Promise(function(resolve, reject) {
        ...
        new targz({}, { fromBase: true })
            .compress(Pencil.documentHandler.tempDir.name, documentPath)
            .then(resolve)
            .catch(reject);
    });
};
