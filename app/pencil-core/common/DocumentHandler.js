...
DocumentHandler.prototype.getAllSupportedExtensions = function(forSaving) {
    ...
    for (var index in fileHandlers) {
        if (
            (forSaving && fileHandlers[index].saveDocument) ||
            (!forSaving && fileHandlers[index].loadDocument)
        )
    ...
    function handler() {
        dialog.showOpenDialog(
            remote.getCurrentWindow(),
            {
                title: "Open",
                defaultPath:
                    Config.get("document.open.recentlyDirPath", null) ||
                    os.homedir(),
                filters: [
                    {
                        name: "Pencil Documents",
                        extensions: thiz.getAllSupportedExtensions(false)
                    }
                ]
            },
            function(filenames) {
...
DocumentHandler.prototype.loadFirstMatchingFromArgs = function(filePaths) {
    const toImport = filePaths
        .map(p => ({
            path: p,
            handler: this.handlerRegistry[path.extname(p)]
        }))
        .find(p => p.handler && p.handler.loadDocument);
    if (toImport) {
        this.loadDocumentFromArguments(toImport.path);
    }
};
...
DocumentHandler.prototype.pickupTargetFileToSave = function(callback) {
    ...
    if (this.controller.documentPath) {
        fileName =
            path
                .basename(this.controller.documentPath)
                .replace(/\.[^\.]+$/, "") + defaultFileType;
...
DocumentHandler.prototype.saveAsDocument = function(
    onSaved,
    skipAddingRecentFiles
) {
    var thiz = this;
    this.pickupTargetFileToSave(function(filePath) {
        ...
        if (!skipAddingRecentFiles)
            thiz.controller.addRecentFile(
                filePath,
                thiz.controller.getCurrentDocumentThumbnail()
            );
...
DocumentHandler.prototype.saveDocument = function(onSaved) {
    if (
        !this.controller.documentPath ||
        !this.getActiveHandler().saveDocument
    ) {
...
DocumentHandler.prototype._saveBoundDocument = function(onSaved) {
    ...
    this.controller.serializeDocument(function() {
        thiz.controller.addRecentFile(
            thiz.controller.documentPath,
            thiz.controller.getCurrentDocumentThumbnail()
        );
        thiz
            .getActiveHandler()
            .saveDocument(thiz.controller.documentPath)
            .then(function() {
                ApplicationPane._instance.unbusy();
                thiz.isSaving = false;
...
DocumentHandler.prototype.newDocument = function() {
    ...
    function create() {
        ...
        setTimeout(function() {
            ...
            var page = thiz.controller.newPage(
                "Untitled Page",
                size.w,
                size.h,
                null,
...
DocumentHandler.prototype.resetDocument = function() {
    ...
    FontLoader.instance.setDocumentRepoDir(
        path.join(this.tempDir.name, "fonts")
    );
};
