...
ShapeXferHelper.prototype.handleData = function (dom) {
    ...
    this.canvas.run(function() {
        ...
        if (this.canvas.currentController.processImportedTransferableProperties) {
            try {
                var processed = this.canvas.currentController.processImportedTransferableProperties();
                if (processed) {
                    this.canvas.selectNone();
                    this.canvas.selectShape(shape);
                }
