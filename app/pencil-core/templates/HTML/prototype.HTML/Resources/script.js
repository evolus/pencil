...
function fitImages() {
    ...
    if (W && H) {
        if (window.lastSize && window.lastSize.W == W && window.lastSize.H == H)
            return;

        var imgs = document.querySelectorAll("body > div.Page img");
        imgs.forEach(function (img) {
            var r = window.useExpandedMode
                ? Math.min(img.naturalWidth / W, img.naturalHeight / H)
                : Math.max(img.naturalWidth / W, img.naturalHeight / H);
...
function generateTOC() {
    ...
    pages.forEach(function (page) {
        ...
        imageWrapper.textContent = page.textContent;
...
function boot() {
    ...
    window.zoomToggleButton.addEventListener(
        "click",
        function () {
